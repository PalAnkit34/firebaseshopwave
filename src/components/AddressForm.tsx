...
export default function AddressForm({ onSubmit, initial, onCancel }: { onSubmit: (a: Omit<Address, 'id'>) => void; initial?: Partial<Address>; onCancel?: () => void }) {
  ...
  const validate = () => {
    ...
    // Simple phone regex to allow for optional country code, but mainly focusing on 10 digits
  ...
  const InputField = ({ name, placeholder, value, onChange, error, type = 'text' }: { name: keyof Address, placeholder: string, value: string, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void, error?: string, type?: string }) => (
    <div>
      <input className={`w-full rounded-lg border px-3 py-2 text-sm ${error ? 'border-red-500' : 'border-gray-300'}`} placeholder={placeholder} value={value} onChange={onChange} type={type} />
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <InputField name="phone" placeholder="Phone*" value={a.phone} onChange={handle('phone')} error={errors.phone} type="tel" />
        <InputField name="pincode" placeholder="Pincode (6 digits)*" value={a.pincode} onChange={handle('pincode')} error={errors.pincode} type="tel" />

...
export default function ProductCard({ p, suggest }: { p: Product; suggest?: any[] }) {
  ...
  return (
    <motion.div 
      ...
      className="card p-3 flex flex-col group"
    >
      <div className="relative">
        <Link href={`/product/${p.slug}`}>
            <div className="relative w-full h-40 overflow-hidden">
                <Image src={p.image} alt={p.name} fill sizes="(max-width: 768px) 50vw, (max-width: 1200px) 25vw, 20vw" className="rounded-lg object-cover transform transition-transform duration-300 group-hover:scale-105"/>
            </div>
        </Link>
      ...
      <div className="flex-grow flex flex-col pt-2">
        ...
        <div className="mt-2 flex gap-2">
          <button onClick={()=>add({ id:p.id, qty:1, price, name:p.name, image:p.image })} className="flex-1 rounded-lg bg-primary py-2 text-primary-foreground text-sm font-semibold transition-colors hover:bg-primary/90">Add to Cart</button>
        </div>
      </div>
